...
describe("buildUpstreamUrl", () => {
  it("strips the route prefix and preserves the remainder plus query", () => {
    expect(buildUpstreamUrl("/api/users/42/orders", "?limit=5")).toBe(
      "https://users.internal/42/orders?limit=5",
    );
  });

  it("maps the bare prefix to the upstream root", () => {
    expect(buildUpstreamUrl("/api/users", "")).toBe("https://users.internal");
  });
