# machi-asia Auth Service

Token issuer service wrapping Supabase Auth for all machi-asia platforms. Built with Next.js.

Authentication itself is hosted Supabase Auth; this service is the server-side boundary that
issues, refreshes, revokes and inspects tokens for frontend repos, and manages the `roles`
claims consumed by `machi-asia/api-gateway` and downstream microservices.
...
- Consumer: `machi-asia/api-gateway` verifies these tokens at the edge and forwards requests with
