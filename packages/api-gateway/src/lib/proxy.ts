...
/**
 * Maps a gateway pathname onto its upstream URL. The full path is forwarded
 * unchanged so each upstream service owns its own namespace (e.g.
 * /api/rose/chat → http://localhost:5000/api/rose/chat).
...
export function buildUpstreamUrl(pathname: string, search: string): string | null {
  ...
  return `${route.upstream}${pathname}${search}`;
}
